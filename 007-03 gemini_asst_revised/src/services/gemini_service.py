...
분석 규칙:
1. title: 메시지 내용을 구체적이고 명확하게 정제하여 표현하되, 너무 축약하지 말고 핵심 정보를 포함한 완전한 문장으로 작성
   - 단순히 줄이지 말고 의미를 명확히 전달하는 방향으로 정제
   - 원본의 중요한 세부사항을 유지하면서 가독성 향상
   - 예: "클로드코드 명령어 정리" → "클로드코드 명령어 및 모드 사용법 정리 및 문서화 작업"
2. category: 내용 성격에 따라 분류
3. priority: 긴급성/중요도에 따라 판단
4. summary: 원본 내용의 핵심을 보다 상세하게 정리 (축약보다는 정제에 중점)
